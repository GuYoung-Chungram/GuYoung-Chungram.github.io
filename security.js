/**
 * ╔══════════════════════════════════════════════════════════╗
 * ║   CHUNGRAM M&C AI Lab — Site Security Module v1.0       ║
...
(function () {
  ...
  document.addEventListener("copy", function (e) {
    // 복사 버튼에 의한 신뢰된 복사는 허용 (프롬프트 복사 버튼 등)
  ...
  document.addEventListener("keydown", function (e) {
    const blocked =
      ...
      (e.ctrlKey && e.key === "U") ||
      (e.ctrlKey && e.key === "S") || // 페이지 저장
      (e.ctrlKey && e.key === "P");   // 인쇄
    if (blocked) {
      e.preventDefault();
  ...
  (function devToolsDetect() {
    ...
    setInterval(function () {
      ...
      if (widthDiff > threshold || heightDiff > threshold) {
        // 블러 처리 — 완전 차단보다 경고 수준 유지
        document.body.style.filter = "blur(8px)";
        showSecurityToast(
          "⚠️ 보안 정책: 개발자 도구 사용이 감지되었습니다.",
          5000
        );
        setTimeout(() => {
  ...
  /* ──────────────────────────────────────────
   * 6. iframe 삽입 방지 (Clickjacking 방어)
   *    (GitHub Pages는 헤더 직접 설정 불가 →
   *     JS로 보완)
   * ────────────────────────────────────────── */
